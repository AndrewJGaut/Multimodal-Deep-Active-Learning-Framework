...
class Experiment:
    ...
    def __init__(self, models, active_learning_methods, test_data_fraction=0.05, final_model_layer_len=64,
        ...
        self.path_to_data = "../kaggle_satellite_image_classification"
        self.num_classes = 4
    ...
    def get_plot_name(self, model_name, active_learning_method):
        return os.path.join("outputs",
                            f"{model_name}_{active_learning_method}.png")
    ...
    def run_experiments(self):
        ...
        for model in self.models:
            for active_learning_method in self.active_learning_methods:
                try:
                    curr_model = model(active_learning_method)
                    print("working on {}".format(curr_model.name()))
                    self.tester.test_model(curr_model)
                    self.plot(curr_model.name(), active_learning_method)
                except Exception as e:
                    print(f"Got exception {e} for model {curr_model.name()}")
